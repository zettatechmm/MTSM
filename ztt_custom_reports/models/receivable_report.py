...
class CustomPartnerBalanceCustomHandler(models.AbstractModel):
    _name = 'account.custom.partner.balance.report.handler'
    _inherit = 'account.report.custom.handler'
    _description = 'Custom Receivable Report Handler'
    ...
    def _report_custom_engine_custom_receivable(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if query_res_lines:
                amount_invoice = query_res_lines[0]['amount_invoice']
                amount_due = query_res_lines[0]['amount_due']
            if current_groupby == 'id':
                ...
                customer_name = self.env['res.partner'].browse(partner_id).name
                invoice_number = query_res['invoice_number'][0] if len(query_res['invoice_number']) == 1 else None
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT move.name) AS invoice_number,
                SUM(move.amount_residual) AS amount_due,
...
class CustomReceivableReportHandler(models.AbstractModel):
    _name = 'custom.receivable.report.handler'
    _inherit = 'account.custom.partner.balance.report.handler'
